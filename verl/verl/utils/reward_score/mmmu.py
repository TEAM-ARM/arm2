...
def compute_score_test(solution_str, ground_truth, method='pass', code_executalbe=False, format_score=0., score=1.):
    ...
    if method == 'pass':
        ...
        if answer is None:
            ...
            # Check if answer is a single character (A-Z or a-z)
            if answer.isalpha() and len(answer) == 1:
                # If it's a single letter, perform case-insensitive comparison
                if answer.upper() == ground_truth.upper():
                    actual_score = score
                else:
                    actual_score = format_score
            else:
                if grade_answer(ground_truth, answer):
                    actual_score = score
                else:
                    actual_score = format_score
    elif method == 'sc':
        if ground_truth.isalpha() and len(ground_truth) == 1:
            if solution_str.upper() == ground_truth.upper():
                actual_score = score
            else:
                actual_score = format_score
        else:
            if grade_answer(ground_truth, solution_str):
                actual_score = score
            else:
                actual_score = format_score
    else:
