...
def get_reasoning_format(solution_str):
...
def compute_ada_grpo_w_length_penalty_outcome_advantage(token_level_rewards: torch.Tensor,
                                               eos_mask: torch.Tensor,
                                               index: torch.Tensor,
                                               sequences_strs: torch.Tensor,
                                               response_length_ind: torch.Tensor,
    ...
    def cosine_decay_reward(reward, current_training_step, total_training_steps):
        b = 1
        reward = b + 0.5 * (reward - b) * (1 + math.cos(math.pi * current_training_step / total_training_steps*0.9))
        return reward
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            id2lengths[uid].append(response_length_ind[i].item())

        for uid, length_list in id2lengths.items():
            id2min_len[uid] = min(length_list)
            id2max_len[uid] = max(length_list)
        ...
        for i in range(bsz):
            ...
            rollout_n = sum(id2format[uid].values())
            cur_format_num = id2format[uid][_format]
            assert cur_format_num > 0, f"cur_format_num: {cur_format_num}"
            alpha = rollout_n / cur_format_num

            _lambda = 0.5
            _epsilon = 0.0001

            l_i = response_length_ind[i]
            l_min = id2min_len[uid]
            l_max = id2max_len[uid]

            denominator = float(l_max - l_min) + _epsilon

            if denominator > _epsilon:
                length_penalty_ratio = (l_i - l_min) / denominator
            else:
                length_penalty_ratio = 0.0

            beta_i = torch.exp(-_lambda * torch.tensor(length_penalty_ratio, device=scores.device, dtype=scores.dtype))
            
            scores[i] = scores[i] * cosine_decay_reward(alpha * beta_i, current_training_step, total_training_steps)
...
def compute_ada_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            _format = get_reasoning_format(sequences_strs[i])
            if _format == "UNKNOWN":
                assert scores[i] == 0., f"score: {scores[i]}, response: {sequences_strs[i]}"
            cur_format_num = id2format[uid][_format]
            assert cur_format_num > 0, f"cur_format_num: {cur_format_num}"
...
def compute_tool_grpo_outcome_advantage(
    execution_passes: torch.Tensor,
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    current_training_step: int,
    ...
    def cosine_decay_alpha(tool_weight,current_training_step, total_training_steps):

        return tool_weight * ( 1 * 1.0 / tool_weight + 0.5 * (1 - 1 * 1.0 / tool_weight) * (1 + math.cos(math.pi * current_training_step / total_training_steps)))   
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if execution_passes[i] == 0:
                alpha = cosine_decay_alpha(tool_weight=1, current_training_step=current_training_step, total_training_steps=total_training_steps)
            else:
                alpha = cosine_decay_alpha(tool_weight=tool_weight, current_training_step=current_training_step, total_training_steps=total_training_steps)
